...
class LMS():
    ...
    def add_course(self,cn,ins,dt,mine,maxe):
    ...
    def register_course(self,cn,email):
           ...
           if len(reg_emp_for_the_course)<int(comp2_val):
                ...
                for i in self.course_registration:
                    if cn in i: 
                       ...
                       allot=f'{i} {emp_email} {cod} {cn} {instructor_name} {date} COURSE_CANCELED'
                       print(allot) 
    ...
    for line in Lines:
        ...
        if query[0]=='ADD-COURSE-OFFERING': 
                   if len(query[1:])==max_args:
                       print(obj.add_course(query[1],query[2],query[3],query[4],query[5]))
                   else:
                       print("INPUT_DATA_ERROR")     
        if query[0]=='REGISTER':
                   if len(query[1:])==min_args:
                       print(obj.register_course(query[2].split('-')[1],query[1]))
                   else:
                       print("INPUT_DATA_ERROR")     
        if query[0]=='ALLOT':
